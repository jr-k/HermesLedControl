...
if [[ "$device" != "don't overwrite existing parameters" ]]; then
    sed -i -e "s/%EXECSTART%/"${escaped}"\/HermesLedControl\/venv\/bin\/python3 main.py --engine="${engine}" --pathToConfig="${pathToConfig}" --hardware="${device}" --debug=True --pattern="${pattern}"/" /etc/systemd/system/hermesledcontrol.service
fi
